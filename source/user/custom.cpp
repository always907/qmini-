...
//        selected_mode = modeSwitcher.get_selected_stick(current_mode);
    ...
    if (selected_mode != current_mode) {
        // 打印模式切换信息
        std::cout << "========================================" << std::endl;
        std::cout << "Mode Switch Detected!" << std::endl;
        std::cout << "Previous mode: " << current_mode << std::endl;
        std::cout << "New mode: " << selected_mode << std::endl;
        ...
        std::cout << "Time: " << time_str << "." << (tv.tv_usec / 1000) << std::endl;
...
void G1::Control() {
    ...
    control_count++;
    if (control_count % 100 == 0) {  // 1秒打印一次
        control_count = 0;
        // 从base_state_buffer_获取最新的RPY数据
        const std::shared_ptr<const BaseState> base_state = base_state_buffer_.GetData();
        if (base_state) {
            std::cout << "q: " << rlController->joint_pos.transpose() << " RPY(deg): " << base_state->rpy[0]*180.0/M_PI << ", " << base_state->rpy[1]*180.0/M_PI << ", " << base_state->rpy[2]*180.0/M_PI << std::endl;
        } else {
            std::cout << "q: " << rlController->joint_pos.transpose() << " RPY(deg): N/A" << std::endl;
        }
    }
}
...
// void G1::IMUStateReader() {
//     static int count = 0;
//     std::cout << "IMUStateReader called #" << ++count << std::endl;
